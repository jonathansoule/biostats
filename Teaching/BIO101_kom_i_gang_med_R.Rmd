...
Programvarene kan du laste ned R direkte fra nettsiden til [The Comprehensive R Archive Network](https://cloud.r-project.org/){target="_blank"} og RStudio fra [RStudio sin nettside](https://rstudio.com/products/rstudio/download/#download){target="_blank"}. 
Pass på at du velger den versjonen som passer din maskin (Windows, MacOS, Linux, etc).
...
I den øvelsen trenger vi pakkene `tidyverse`, `learnr`, `shiny`and `rmarkdown`. 
De installeres via konsolen ved hjelp av funksjonen `install.packages()`. Kopiér og lim inn den følgende linjen direkte i konsolen, og trykk `Enter`:
...
Skriv en innledning i to eller tre setninger om hva studien handler om.
